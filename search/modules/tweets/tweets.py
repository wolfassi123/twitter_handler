...
class Tweet:
    ...
    def getRecentTweet(self, query: str, max: int):
        response = self.__client.search_recent_tweets(
            query=query, max_results=max, user_auth=True
        )
    ...
    ## This function will return all of recent tweets that match a specific query.
    def getRecentTweetInfo(self, query: str, max: int):
        response = self.__client.search_recent_tweets(
            query=query, max_results=max, user_auth=True
        )
        tweetsdata = response["data"]
    ...
    def getTweetInfo(self, id: int, keys_list: list = []):
        ...
        for key in keys_list:
            if isinstance(key, str):
                ...
                if isinstance(tweetdata[key], datetime):
                    tweet[key] = tweetdata[key].strftime("%m/%d/%Y, %H:%M:%S")
        return tweet
    ...
    def getTweetBasic(self, id: int):
        response = self.__client.get_tweet(
            id=id, tweet_fields=["public_metrics"], expansions=["author_id"]
        )
        tweetdata = response["data"]
    ...
    def getTweetAnnotations(self, id: int):
        response = self.__client.get_tweet(
            id=id, tweet_fields=["context_annotations", "entities", "reply_settings"]
        )
        tweetdata = response["data"]
        tweet = dict()
